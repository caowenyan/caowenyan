...
public class Test18 {
    ...
    /**
     * 判断当前节点的root1是否包含roo2
     */
    public static boolean equalSubTree(BinaryTreeNode root1, BinaryTreeNode root2) {
        // 若是子元素已经遍历完成，则是true
        if (root2 == null) {
            return true;
        }
        // 若子元素未遍历完但是主元素遍历完了则是false
        if (root1 == null) {
            return false;
        }
        if (root1 == root2) {
        ...
        // 若有子节点判断子节点是否相等，没有直接返回true
        return equalSubTree(root1.left, root2.left) && equalSubTree(root1.right, root2.right);
    }
